...
pacman::p_load("knitr","ggplot2","readxl","magrittr","tidyverse")
...
#from the graph, we can see that the unit price for non-organ strawberry is growing rapidly over the years, no matter the states.
...
for (i in unique(name)){
  df_f <- plot_data_fert %>% filter(plot_data_fert$name == i)

  out <- ggplot(df_f, aes(fill=year, y=value, x=state)) + 
    geom_bar(position="dodge", stat="identity") +
    ggtitle(paste("fertlizer usage", i, " (LB / ACRE / YEAR)")) +
    ylab('value')
    print(out)
}

#The fertilizer usage on strawberry experienced a substantial decreasing in California, while it increased slightly in Florida')
```
